...
function adminLogin() {
  motDePasse = document.getElementById("admin-pwd").value;
  socket.emit("admin_auth", motDePasse);
}

socket.on("admin_ok", () => {
  document.getElementById("zone-login").style.display = "none";
  document.getElementById("zone-dashboard").style.display = "block";
  toast("Connecté en tant que maître du jeu");
});
...
function validate() {
  ...
  // Envoi au serveur
  socket.emit("admin_pin_attempt", { pin: pin });

  // Écoute la réponse
  socket.on("admin_pin_response", (response) => {
      if (response.success) {
          setStatus('Code Accepté', 'success');
          setTimeout(() => {
              window.location.href = '../accueil_admin/admin.html';
          }, 600);
      } else {
